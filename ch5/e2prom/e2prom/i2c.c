...
int i2c_write16a_bytes(uint8_t dev, uint16_t addr, uint8_t *data, int len)
{
	uint8_t sub_addr[2];
	
	if(i2c_set_start()==0){
		if(i2c_set_dev_addr(dev, 0)==0){
			sub_addr[0] = (uint8_t)((addr/0x100)>>8);
			sub_addr[1] = (uint8_t)(addr&0xff);
			if(i2c_mt_send_data(sub_addr, 2)==0){
				if(i2c_mt_send_data(data, len)==0){
					if(i2c_set_stop()==0)
					return 0;
				}
...
int i2c_read16a_bytes(uint8_t dev, uint16_t addr, uint8_t *data, int len)
{
	uint8_t sub_addr[2];
